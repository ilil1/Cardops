...
`cardops_ai.scripts.import_customers --replace`로 적재합니다(customers와
연관 데이터를 모두 지우고 교체).
"""
...
from cardops_ai.app.config import get_model_dir
from cardops_ai.app.model_registry import ModelRegistry
...
DEFAULT_OUTPUT_PATH = (
    Path(__file__).resolve().parents[4] / "data" / "synthetic" / "synthetic_customers.csv"
)

# 모델(`PredictionRequest`, backend/ai-service/cardops_ai/app/schemas.py)이 허용하는 유효 범위입니다.
...
def main() -> None:
    ...
    print(
        "적재하려면: docker compose run --rm jobs python -m cardops_ai.scripts.import_customers "
