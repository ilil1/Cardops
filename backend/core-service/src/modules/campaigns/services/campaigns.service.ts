...
import {
  ...
  booleanValue, campaignResponse, campaignStats, dateValue, fail, insertedId,
...
export class CampaignsService {
  constructor(private readonly events: CampaignEventsRepository, private readonly unitOfWork: UnitOfWork, private readonly repository: CampaignsRepository) {}

  async campaign(db: SqlClient | undefined, id: number, lock = false): Promise<Row | null> {
    return this.repository.findCampaign(lock, id, db);
  }

  async target(db: SqlClient | undefined, id: number, lock = false): Promise<Row | null> {
    return this.repository.findTarget(lock, id, db);
  }

  private async statsFor(db: SqlClient | undefined, campaignId: number): Promise<Row> {
    return campaignStats(await this.repository.targetStats(campaignId, db));
  }

  async listCampaigns(query: Row): Promise<Row> {
    const { page, pageSize, offset } = pageQuery(query);
    if (query.status !== undefined && !CAMPAIGN_STATES.includes(query.status)) fail(422, 'Invalid campaign status.');
    const filters = { ...query, created_by_user_id: query.created_by_user_id === undefined
      ? undefined : positiveId(query.created_by_user_id) };
    const { count, rows } = await this.repository.campaignPage(filters, pageSize, offset);
    const items = await Promise.all(rows.map(async (row) => campaignResponse(row, await this.statsFor(undefined, row.id))));
  ...
  async getCampaign(id: number): Promise<Row> {
    const row = await this.campaign(undefined, id);
    if (!row) fail(404, 'The campaign was not found.');
    return campaignResponse(row, await this.statsFor(undefined, id));
  }
  ...
  async createCampaign(payload: Row, actor: Actor): Promise<Row> {
    ...
    checkExperiment(row);
    try {
      const id = await this.unitOfWork.transaction(async (tx) => {
        const result = await this.repository.insertCampaign(row.name, row.description, row.channel, row.segment_code, row.start_at, row.end_at, row.experiment_enabled, row.control_group_ratio, seed(), row.fixed_cost, row.cost_per_contact, row.revenue_per_conversion, row.retention_window_days, actor.id, tx);
  ...
  async updateCampaign(id: number, payload: Row, actor: Actor): Promise<Row> {
    ...
    try {
      await this.unitOfWork.transaction(async (tx) => {
        ...
        const targets = await this.repository.lockCampaignTargets(id, tx);
        ...
        if (next.name !== current.name && targets.length)
          await this.repository.renameTargetCampaign(next.name, id, tx);
        if (next.status === 'cancelled' && current.status !== 'cancelled') {
          for (const target of targets.filter((item) => OPEN_TARGET_STATES.includes(item.status))) {
            await this.repository.updateTargetStatus('cancelled', new Date(), target.id, tx);
            await this.events.append(tx, id, actor.id, 'status_changed', { target_id: target.id,
              from_status: target.status, to_status: 'cancelled', note: 'Cancelled when the campaign was cancelled.' });
          }
        }
        await this.repository.updateCampaign(next.name, next.description, next.channel, next.segment_code, next.status, dateValue(next.start_at), dateValue(next.end_at), next.experiment_enabled, next.control_group_ratio, next.fixed_cost, next.cost_per_contact, next.revenue_per_conversion, next.retention_window_days, id, tx);
        if (next.status !== current.status)
          await this.events.append(tx, id, actor.id, 'status_changed', { from_status: current.status, to_status: next.status });
      });
      return this.getCampaign(id);
    } catch (error) {
      if (isDuplicate(error)) fail(409, 'A campaign with the same name already exists.');
  ...
  async listTargets(query: Row): Promise<Row> {
    const { page, pageSize, offset } = pageQuery(query);
    if (query.campaign_id !== undefined && !(await this.campaign(undefined, positiveId(query.campaign_id))))
      fail(404, 'The campaign was not found.');
    if (query.status !== undefined && !TARGET_STATES.includes(query.status)) fail(422, 'Invalid target status.');
    const filters: Row = { ...query, converted: optionalBoolean(query.converted) };
    for (const field of ['campaign_id', 'assigned_to_user_id', 'customer_id']) {
      if (query[field] !== undefined) filters[field] = positiveId(query[field]);
    }
    const { count, allForStats, rows } = await this.repository.targetPage(filters, pageSize, offset);
    const total = numberValue(count?.total);
  ...
  async listEvents(campaignId: number, query: Row): Promise<Row> {
    if (!(await this.campaign(undefined, campaignId))) fail(404, 'The campaign was not found.');
    const { page, pageSize, offset } = pageQuery(query, 50);
    const targetId = query.campaign_target_id === undefined ? undefined : positiveId(query.campaign_target_id);
    const { count, rows } = await this.repository.eventPage(campaignId, targetId, pageSize, offset);
  ...
  async slaSummary(): Promise<Row> {
    const rows = await this.repository.contactDurations();
  ...
  async createTarget(payload: Row, actor: Actor): Promise<Row> {
    ...
    try {
      const id = await this.unitOfWork.transaction(async (tx) => {
        const insight = await this.repository.findInsight(insightId, tx);
        ...
        if (payload.campaign_id) {
          ...
          campaign = await this.repository.lockCampaignByName(name, tx);
          if (!campaign) {
            const result = await this.repository.insertLegacyCampaign(name, 'draft', actor.id, tx);
            const campaignId = insertedId(result);
            await this.events.append(tx, campaignId, actor.id, 'created', { to_status: 'draft',
        ...
        if (payload.assigned_to_user_id !== undefined && payload.assigned_to_user_id !== null) {
          assignee = await this.repository.findAssignee(positiveId(payload.assigned_to_user_id), tx);
          if (!assignee) fail(404, 'The assigned user was not found.');
        }
        return this.createTargetInTx(tx, campaign!, insight, assignee, actor);
      });
      const target = await this.target(undefined, id);
  ...
  async createTargetInTx(db: SqlClient | undefined, campaign: Row, insight: Row, assignee: Row | null,
    ...
    const customer = await this.repository.lockCustomer(customerId, db);
    ...
    const recent = await this.repository.findRecentContact(customerId, cutoff, db);
    if (recent) fail(409, 'The customer was contacted within the contact cooldown period.');
    const duplicate = await this.repository.findDuplicateTarget(customerId, campaign.id, db);
    if (duplicate) fail(409, 'The customer is already registered in this campaign.');
    const open = await this.repository.lockOpenTargets(customerId, db);
    ...
    for (const existing of open) {
      await this.repository.updateTargetStatus('cancelled', new Date(), existing.id, db);
      await this.events.append(db, existing.campaign_id, actor.id, 'status_changed', {
    ...
    const result = await this.repository.insertTarget(customerId, insight.id, campaign.id, bulkRunId, campaign.name, experimentGroup, assignedId, status, db);
    const id = insertedId(result);
    await this.events.append(db, campaign.id, actor.id, 'created', { target_id: id, to_status: status });
    if (assignedId) await this.events.append(db, campaign.id, actor.id, 'assigned', {
  ...
  async updateTarget(id: number, payload: Row, actor: Actor): Promise<Row> {
    ...
    await this.unitOfWork.transaction(async (tx) => {
      ...
      if (provided('assigned_to_user_id')) {
        ...
        await this.events.append(tx, campaign.id, actor.id, 'assigned', { target_id: id, from_status: previous,
      ...
      if (next !== previous) {
        ...
        if (['contacted', 'completed'].includes(next)) {
          ...
          await this.repository.updateLastContact(now, target.customer_id, tx);
        }
        ...
        await this.events.append(tx, campaign.id, actor.id, 'status_changed', { target_id: id,
      ...
      if (resultCode === 'opted_out')
        await this.repository.optOutCustomer(target.customer_id, tx);
      ...
      if (changedResult) await this.events.append(tx, campaign.id, actor.id,
      ...
      if (Object.keys(updates).length) {
        await this.repository.updateTarget(updates, id, tx);
      }
    });
    return targetResponse((await this.target(undefined, id))!);
  }
  ...
  async updateContactPreferences(customerId: number, payload: Row): Promise<Row> {
    ...
    await this.repository.updateOptOut(payload.marketing_opt_out, customerId);
    const updated = await this.repository.contactPreferences(customerId);
