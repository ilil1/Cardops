...
import {
  ...
  SqlClient, addEvent, booleanValue, dateOnly, fail, insertedId, isDuplicate,
...
export class BulkTargetingService {
  constructor(private readonly db: DatabaseService, private readonly campaigns: CampaignsService) {}

  private async buildRules(db: SqlClient, payload: Row, overrideSeed?: string): Promise<Row> {
    const where = ["status = 'succeeded'"], params: unknown[] = [];
    if (payload.scoring_batch_id !== undefined && payload.scoring_batch_id !== null) {
      where.push('id = ?'); params.push(positiveId(payload.scoring_batch_id));
    }
    if (payload.source_as_of_date) { where.push('as_of_date <= ?'); params.push(payload.source_as_of_date); }
    const batch = await db.one<Row>(`SELECT * FROM scoring_batches WHERE ${where.join(' AND ')}
      ORDER BY as_of_date DESC, completed_at DESC, id DESC LIMIT 1`, params);
    ...
    if (payload.segment === 'medium_reactivation') {
      const gaps = await db.query<Row>('SELECT activity_gap FROM customer_insights WHERE scoring_batch_id = ?', [batch.id]);
      rules.activity_gap_threshold = quantile(gaps.map((row) => numberValue(row.activity_gap)), rules.activity_gap_quantile);
    }
    return rules;
  }

  private async scan(db: SqlClient, rules: Row): Promise<Scan> {
    const rows = await db.query<Row>(`SELECT i.*, c.total_revolving_bal, c.total_trans_ct, c.total_trans_amt,
      ...
      WHERE i.scoring_batch_id = ?`, [rules.scoring_batch_id]);
    ...
    const targets = await db.query<Row>(`SELECT t.customer_id, t.status, t.processed_at, c.segment_code,
  ...
  private async previewInTx(db: SqlClient, payload: Row, actor: Actor, rerunOfId?: number,
    ...
    if (payload.assigned_to_user_id !== undefined && payload.assigned_to_user_id !== null) {
      const assignee = await db.one<Row>('SELECT role, is_active FROM users WHERE id = ?', [positiveId(payload.assigned_to_user_id)]);
    ...
    const result = await db.execute(`INSERT INTO bulk_targeting_runs
      ...
      VALUES (?, 'previewed', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [rules.segment, actor.id, rerunOfId ?? null, rules.scoring_batch_id,
        rules.source_as_of_date, JSON.stringify(rules), selectedIds.size, scan.eligible.length,
        scan.skippedActive, scan.skippedRecent, scan.skippedOptOut]);
    const runId = insertedId(result);
    for (const [index, candidate] of scan.evaluated.entries()) {
      await db.execute(`INSERT INTO bulk_targeting_candidates
        (run_id, customer_id, customer_insight_id, \`rank\`, eligible, selected, exclusion_reason, execution_status)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')`,
        [runId, candidate.insight.customer_id, candidate.insight.id, index + 1,
          candidate.exclusion === null, selectedIds.has(numberValue(candidate.insight.customer_id)), candidate.exclusion]);
    }
    return runId;
  }

  async preview(payload: Row, actor: Actor): Promise<Row> {
    const id = await this.db.transaction((tx) => this.previewInTx(tx, payload ?? {}, actor));
    return this.get(id);
  }

  private async run(db: SqlClient, id: number, lock = false): Promise<Row | null> {
    return db.one<Row>(`SELECT r.*, c.status AS campaign_status FROM bulk_targeting_runs r
      LEFT JOIN campaigns c ON c.id = r.campaign_id WHERE r.id = ? ${lock ? 'FOR UPDATE' : ''}`, [id]);
  }

  private async response(db: SqlClient, run: Row, includePreview: boolean): Promise<Row> {
    ...
    const selected = includePreview ? await db.query<Row>(`SELECT i.* FROM bulk_targeting_candidates s
      JOIN customer_insights i ON i.id = s.customer_insight_id
      WHERE s.run_id = ? AND s.selected = 1 ORDER BY s.\`rank\``, [run.id]) : [];
    return {
  ...
  async list(query: Row): Promise<Row> {
    ...
    const count = await this.db.one<Row>('SELECT COUNT(*) AS total FROM bulk_targeting_runs');
    const runs = await this.db.query<Row>(`SELECT r.*, c.status AS campaign_status FROM bulk_targeting_runs r
      LEFT JOIN campaigns c ON c.id = r.campaign_id ORDER BY r.created_at DESC, r.id DESC LIMIT ? OFFSET ?`, [pageSize, offset]);
    const items = await Promise.all(runs.map((run) => this.response(this.db, run, false)));
  ...
  async get(id: number): Promise<Row> {
    const run = await this.run(this.db, id);
    if (!run) fail(404, 'The bulk targeting run was not found.');
    return this.response(this.db, run, true);
  }
  ...
  async execute(id: number, actor: Actor): Promise<Row> {
    try {
      await this.db.transaction(async (tx) => {
        ...
        const snapshots = await tx.query<Row>(`SELECT * FROM bulk_targeting_candidates
          WHERE run_id = ? AND selected = 1 ORDER BY \`rank\` FOR UPDATE`, [id]);
        if (!snapshots.length) fail(409, 'The preview has no selected candidates to execute.');
        const rules = jsonValue(run.rules_json);
        const inserted = await tx.execute(`INSERT INTO campaigns
          ...
          [rules.campaign_name, rules.description, rules.channel, run.segment_code,
            booleanValue(rules.experiment_enabled), numberValue(rules.control_group_ratio),
            rules.experiment_seed ?? seed(), numberValue(rules.fixed_cost), numberValue(rules.cost_per_contact),
            numberValue(rules.revenue_per_conversion), numberValue(rules.retention_window_days, 30), actor.id]);
        const campaignId = insertedId(inserted);
        await tx.execute('UPDATE bulk_targeting_runs SET campaign_id = ?, status = ?, executed_at = ? WHERE id = ?',
          [campaignId, 'executed', new Date(), id]);
        ...
        const assignee = rules.assigned_to_user_id ? await tx.one<Row>('SELECT * FROM users WHERE id = ?', [rules.assigned_to_user_id]) : null;
        let created = 0;
        for (const snapshot of snapshots) {
          const insight = await tx.one<Row>('SELECT * FROM customer_insights WHERE id = ?', [snapshot.customer_insight_id]);
          if (!insight) { await this.skipSnapshot(tx, snapshot.id, null); continue; }
          const customer = await tx.one<Row>('SELECT * FROM customers WHERE customer_id = ? FOR UPDATE', [snapshot.customer_id]);
          ...
          try {
            ...
            await tx.execute('UPDATE bulk_targeting_candidates SET execution_status = ?, campaign_target_id = ? WHERE id = ?',
              ['created', targetId, snapshot.id]);
            created++;
          } catch (error) {
        ...
        await tx.execute('UPDATE bulk_targeting_runs SET created_count = ? WHERE id = ?', [created, id]);
      });
      return this.get(id);
    } catch (error) {
  ...
  private async skipSnapshot(tx: SqlClient, id: number, reason: string | null): Promise<void> {
    await tx.execute('UPDATE bulk_targeting_candidates SET execution_status = ?, exclusion_reason = ? WHERE id = ?',
      ['skipped', reason, id]);
  }
  ...
  async cancel(id: number, actor: Actor): Promise<Row> {
    await this.db.transaction(async (tx) => {
      ...
      if (run.status === 'previewed') {
        await tx.execute(`UPDATE bulk_targeting_candidates SET execution_status = 'cancelled'
          WHERE run_id = ? AND selected = 1 AND execution_status = 'pending'`, [id]);
        await tx.execute('UPDATE bulk_targeting_runs SET status = ?, cancelled_at = ? WHERE id = ?', ['cancelled', now, id]);
        return;
      }
      const targets = await tx.query<Row>(`SELECT * FROM campaign_targets WHERE bulk_targeting_run_id = ?
        AND status IN ('pending', 'assigned') FOR UPDATE`, [id]);
      for (const target of targets) {
        ...
        await addEvent(tx, target.campaign_id, actor.id, 'status_changed', { target_id: target.id,
          from_status: target.status, to_status: 'cancelled', note: 'Cancelled by bulk targeting run.' });
        await tx.execute(`UPDATE bulk_targeting_candidates SET execution_status = 'cancelled'
          WHERE run_id = ? AND campaign_target_id = ?`, [id, target.id]);
      }
      await tx.execute(`UPDATE bulk_targeting_runs SET status = 'cancelled', cancelled_at = ?,
        cancelled_target_count = cancelled_target_count + ? WHERE id = ?`, [now, targets.length, id]);
    });
    return this.get(id);
  }
